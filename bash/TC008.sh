...
result=$(redis-cli -h $ADVERTISEMENTS_HOST -p $ADVERTISEMENTS_PORT HMSET ad_9407e8c8ba5042cea891c351a5672107 flight_id 'bc64d529356b4682b51dca09dfb9a31f' cost_received '0.000000' cost_paid '1.000000')

# Flight
result=$(redis-cli -h $FLIGHTS_HOST -p $FLIGHTS_PORT DEL fl_bc64d529356b4682b51dca09dfb9a31f)
result=$(redis-cli -h $FLIGHTS_HOST -p $FLIGHTS_PORT HMSET fl_bc64d529356b4682b51dca09dfb9a31f flight_name 'Balloon Standard iTVC Testing' buy_type 'cpm' added_value '0.00' bookings_type 'impression' daily_bookings '1' campaign_id '7eb3e2fc77e04bc984eb548eba50b817' banner_id '02075b34df2e4facba222da65d3cbbcc' delivery_id 'eebe98f29524431faf52d43732426ec3' category_id '51b6bb2730b74b7cb7f15c1bdaf075b4' product_id '88a1909a30b443f3b01993755ce1c640' audience_id 'fb8d61f9890a415b8756dd7fce6a1c54' bookings '1000.00')
...
# Banner
result=$(redis-cli -h $BANNERS_HOST -p $BANNERS_PORT DEL b_02075b34df2e4facba222da65d3cbbcc)
result=$(redis-cli -h $BANNERS_HOST -p $BANNERS_PORT HMSET b_02075b34df2e4facba222da65d3cbbcc creative_source '{"source":"2017\/06\/ultherapy\/balloon-standard-op1\/index.html","width":"300","height":"250","bar_height":"250","duration":"30"}' ad_format 'c948c8813cfd4f77968744b3ab18111c' banner_type 'template' template_name 'display_balloon_standard_itvc' landing_page 'http://yomedia.vn')

# Ad Format
result=$(redis-cli -h $ADFORMATS_HOST -p $ADFORMATS_PORT DEL adf_c948c8813cfd4f77968744b3ab18111c)
result=$(redis-cli -h $ADFORMATS_HOST -p $ADFORMATS_PORT HMSET adf_c948c8813cfd4f77968744b3ab18111c format_name 'Display Balloon Standard (iTVC) (VN)' tags 'balloon' video '1')
